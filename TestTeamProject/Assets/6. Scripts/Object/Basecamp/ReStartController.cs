...
public class ReStartController : InteractableObject
{
    ...
    public override IEnumerator InteractWithObject(GameObject player)
    {
        ...
        foreach (NetworkConnectionToClient conn in NetworkServer.connections.Values)
        {
            ...
            while (Vector3.Distance(conn.identity.gameObject.transform.position, GameObject.Find("GameDataManager").GetComponent<GameDataController>().spawnPoint.transform.GetChild(index).position) > 10f
                && conn.identity != null)
            {
